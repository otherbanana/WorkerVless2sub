...
// 设置优选TLS地址，不带端口号默认8443addressesapi
let addresses = [
	'yx.lxfproxy.top:443#优选域名-电信',
    'cfip.xxxxxxxx.tk:443#优选域名-OTC提供',
    'cf.090227.xyz:443#优选域名-CM提供',
];
...
let addressesapi = [
	...
	//'https://cf-workers-text2kv-ltd.pages.dev/ip.txt?token=banana',
	'https://gist.githubusercontent.com/otherbanana/31c8d25e2797ff0172dede78d99d3ec3/raw',
...
let subconverter = "psub.otherbanana.workers.dev"; //在线订阅转换后端，目前使用肥羊的订阅转换功能。支持自建psub 可自行搭建https://github.com/bulianglin/psub
let subconfig = "https://raw.githubusercontent.com/cmliu/ACL4SSR/main/Clash/config/ACL4SSR_Online_Full_MultiMode.ini"; //订阅配置文件

let BotToken ='1894230309:AAHYbBksQJh_3IynzZ-CWRspYay3ydHscIM'; //可以为空，或者@BotFather中输入/start，/newbot，并关注机器人
let ChatID ='260415029'; //可以为空，或者@userinfobot中获取，/start
let vmessLinks = [
	'vmess:',
];
...
async function getAddressesapi() {
	...
	try {
		...
		const apiAddresses = lines.map((line, index) => {
			const match = line.match(regex);
			if (match) {
				const ipAddress = match[0];
				const operator = line.endsWith('CT') ? 'CT' : line.endsWith('CMCC') ? 'CMCC' : 'Unknown';
				return { ipAddress, operator, index: index + 1 };
			}
			return null;
		}).filter(Boolean);
